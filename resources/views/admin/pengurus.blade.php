...
@section('content')
    ...
    @else
        <x-card title="Data Pengurus">
            <x-text-field disabled label="Ketua RW" name="ketua_rw" value="{{ $data->ketua_rw }}" required
                placeholder="Nama Ketua RW"></x-text-field>
            <x-text-field disabled label="Ketua RT" name="ketua_rt" value="{{ $data->ketua_rt }}" required
                placeholder="Nama Ketua RT"></x-text-field>
            <div class="row">
                <div class="col-lg-6">
                    <x-text-field disabled label="Sekretaris" name="sekretaris" value="{{ $data->sekretaris }}" required
                        placeholder="Nama Sekretaris"></x-text-field>
                </div>
                <div class="col-lg-6">
                    <x-text-field disabled label="Bendahara" name="bendahara" value="{{ $data->bendahara }}" required
                        placeholder="Nama Bendahara"></x-text-field>
                </div>
            </div>
        </x-card>
        ...
        <x-card title="Seksi-Seksi">
            @foreach ($data->seksi_seksi as $key => $item)
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        <x-text-field disabled label="Bagian ke-{{ $loop->iteration }}"
                            placeholder="Bagian Kerja (Keamanan / Keagamaan / dll)" value="{{ $item->bagian }}"
                            name="seksi_seksi[{{ $key }}][bagian]"></x-text-field>
                    </div>
                    <div class="col-lg-6">
                        <x-text-field disabled label="Petugas" placeholder="Nama Petugas" value="{{ $item->petugas }}"
                            name="seksi_seksi[{{ $key }}][petugas]">
                        </x-text-field>
                    </div>
                </div>
